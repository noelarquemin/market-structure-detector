...
if ticker in st.session_state.history:
    ...
    setups = detect_setups(he, strategy_level)
    ...
    fig = plot_market_structure_interactive(
        ...
        trades=trades if show_trades_on_chart else None,
    )
    ...
    # --- Section stratégie ---
    st.divider()
    st.subheader(f"📊 Stratégie — niveau {strategy_level}")
    ...
    if trades.empty:
        ...
        c2.metric("Winrate", f"{summary['winrate']*100:.1f}%" if summary["n_trades"] - summary["n_open"] > 0 else "—")
